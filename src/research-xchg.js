...
// class list the game dumps at boot, and the exchange directory where the
...
const CLASSES_FILE = 'classes.txt';
const RETRY_MS = 200;
...
export class ResearchXchg {
  ...
  readClasses() {
    ...
    if (!existsSync(p)) return { names: [], count: 0, at: '' };
    const names = readFileSync(p, 'utf8').split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
    return { names, count: names.length, at: stampNow(new Date(statSync(p).mtimeMs)) };
  }
  ...
  sweep(keepTokens) {
    ...
    for (const file of readdirSync(this.xchgDir)) {
      let drop = false;
      if (file.endsWith('.part')) drop = true;
      else {
