...
try {
  ...
  const boot = await call('/v1/research/boot', { Revision: 1, Counters: 'owners=1', Names: NAMES });
  ok('boot reads the files that exist', boot, { ok: true, configs: 2, versions: 2 });
  ...
  ok('the log says so', log.includes('[research] boot from research-test: 2 configs (2 new versions), revision 1'), true);
  ...
  ok('classes is no research op any more', (await admin('classes')).why, 'unknown op: classes');
  ok('status knows the booted server', (await admin('status')).booted.map((b) => [b.id, b.revision]), [['research-test', 1]]);
